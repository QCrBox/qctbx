...
class MATTSF0jSource(F0jSource):
    ...
    def calc_f0j(
        ...
        with open(os.path.join(self.work_folder, f'{self.filebase}_cli.out'), 'w') as fobj:
            subprocess.run([self.discamb_path], cwd=self.work_folder, stdout=fobj, check=True)
